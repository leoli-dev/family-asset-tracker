...
import { AssetRecord, AssetCategory, Language } from '../types';
...
export const HistoryTable: React.FC<HistoryTableProps> = ({ records, onDelete, isDemoMode, language }) => {
  ...
  const uniqueAccounts = useMemo(() => Array.from(new Set(records.map(r => r.accountName))).sort(), [records]);
  const uniqueCategories = useMemo(() => Array.from(new Set(records.map(r => r.category))).sort(), [records]);
  const uniqueOwners = useMemo(() => Array.from(new Set(records.map(r => r.owner))).sort(), [records]);
  ...
  const filteredRecords = useMemo(() => {
    return sortedRecords.filter(record => {
        const matchesAccount = filterAccount ? record.accountName === filterAccount : true;
        const matchesCategory = filterCategory ? record.category === filterCategory : true;
        const matchesOwner = filterOwner ? record.owner === filterOwner : true;
        ...
        <div className="grid grid-cols-1 gap-3">
            <div className="grid grid-cols-2 gap-3">
                <select
                    ...
                    className="w-full p-2.5 text-sm bg-slate-50 border border-slate-200 rounded-lg outline-none focus:ring-2 focus:ring-blue-500 text-slate-700"
                >
            ...
            >
                ...
                {uniqueCategories.map(c => <option key={c} value={c}>{CATEGORY_LABELS[language][c]}</option>)}
            </select>
        </div>
      </div>
      ...
      <div className="space-y-3">
        {paginatedRecords.map((record) => (
          <div key={record.id} className="bg-white p-4 rounded-xl shadow-sm border border-slate-100 flex flex-col gap-3 relative">
            ...
            {/* Top Row: Account Name & Amount */}
            <div className="flex justify-between items-start">
              <div>
                <div className="font-bold text-slate-800 text-lg leading-tight">{record.accountName}</div>
                {record.note && <div className="text-xs text-slate-400 mt-1 italic line-clamp-1">{record.note}</div>}
              </div>
              <div className="text-right">
                <div className={`font-bold text-lg ${record.category === AssetCategory.LIABILITY ? 'text-red-500' : 'text-slate-800'}`}>
                  {record.amount.toLocaleString()} 
                </div>
                <div className="text-xs font-bold text-slate-400">{record.currency}</div>
              </div>
            </div>
            ...
            <div className="flex justify-between items-center">
              ...
              <div className="flex items-center gap-3">
                ...
                >
                    {CATEGORY_LABELS[language][record.category]}
                </span>
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between p-2 mt-4">
            ...
            <span className="text-sm font-medium text-slate-500">
                {t('history.page', language)} {currentPage} {t('history.of', language)} {totalPages}
            </span>
            <button 
